...
THRESHOLD_OUTPUT_PATH = Path("reports/threshold_analysis.csv")
SELECTED_THRESHOLD_OUTPUT_PATH = Path("reports/selected_threshold.csv")

FIGURES_DIR = Path("reports/figures/model")
THRESHOLD_PLOT_PATH = FIGURES_DIR / "threshold_analysis.png"
...
def select_best_threshold(results_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if eligible_df.empty:
        print(
            "\nWarning: No threshold satisfies the minimum approval-rate rule. "
            "Selecting threshold with highest F1 score instead."
        )
...
def plot_threshold_analysis(
    ...
    plt.title("Threshold Analysis Using Calibrated Probability of Default")
